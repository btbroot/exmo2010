...
def task_permission(user, priv, task):
    ...
    if priv == 'exmo2010.view_task':
        ...
        if task.open and task.user == user and (task.organization.monitoring.is_rate or task.organization.monitoring.is_revision): return True
    ...
    if priv == 'exmo2010.exclude_parameter':
        if user.is_active:
            ...
            if user.profile.is_expertB \
              and (parameter.monitoring.is_rate or parameter.monitoring.is_interact parameter.monitoring.is_revision):
                return True
    return False
