...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        if not db.dry_run:
            for monitoring in orm.Monitoring.objects.all():
                dates = {}
                for status in migrate_statuses:
                    m_status = get_object_or_None(
                        orm.MonitoringStatus,
                        monitoring= monitoring,
                        status = status
                        )
                    if m_status:
                        dates[status] = m_status.start
                monitoring.prepare_date = dates.get(Monitoring.MONITORING_PREPARE)
                monitoring.rate_date = dates.get(Monitoring.MONITORING_RATE)
                monitoring.revision_date = dates.get(Monitoring.MONITORING_REVISION)
                monitoring.interact_date = dates.get(Monitoring.MONITORING_INTERACT)
                monitoring.result_date = dates.get(Monitoring.MONITORING_RESULT)
                monitoring.finishing_date = dates.get(Monitoring.MONITORING_FINISHING)
                monitoring.publish_date = dates.get(Monitoring.MONITORING_PUBLISH)
                monitoring.save()
    ...
    def backwards(self, orm):
        ...
        if not db.dry_run:
            for monitoring in orm.Monitoring.objects.all():
                dates = {}
                dates[Monitoring.MONITORING_PREPARE] = monitoring.prepare_date
                dates[Monitoring.MONITORING_RATE] = monitoring.rate_date
                dates[Monitoring.MONITORING_INTERACT] = monitoring.interact_date
                dates[Monitoring.MONITORING_RESULT] = monitoring.result_date
                dates[Monitoring.MONITORING_FINISHING] = monitoring.finishing_date
                dates[Monitoring.MONITORING_PUBLISH] = monitoring.publish_date
                for status in migrate_statuses:
                    m_status, created = orm.MonitoringStatus.objects.get_or_create(
                        monitoring = monitoring,
                        status = Monitoring.MONITORING_PREPARE
                    )
                    m_status.start = dates.get(status)
                    m_status.save()
