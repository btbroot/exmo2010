...
class Migration(DataMigration):
    """
        type:
            ...
            2 -- digest with duratation
    """
    def forwards(self, orm):
        import json
        for profile in orm.UserProfile.objects.all():
            notify_comment_type = 0
            if profile.notify_comment: notify_comment_type = 1
            notify_score_type = 0
            if profile.notify_score_change: notify_score_type = 1

            pref = {
                'notify_comment': {
                    'type': notify_comment_type,
                    'digest_duratation': 5,
                    'self': profile.notify_self_comment,
                },
                'notify_score': {
                    'type': notify_score_type,
                    'digest_duratation': 5,
                }
            }

            profile.preference = json.dumps(pref)
            profile.save()
