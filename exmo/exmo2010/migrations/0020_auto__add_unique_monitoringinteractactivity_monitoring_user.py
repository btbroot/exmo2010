...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding unique constraint on 'MonitoringInteractActivity', fields ['monitoring', 'user']
        if not db.dry_run:
            for row in orm.MonitoringInteractActivity.objects.all():
                if orm.MonitoringInteractActivity.objects.filter(
                    user=row.user,
                    monitoring=row.monitoring,
                ).count() > 1:
                    row.delete()

        db.create_unique('exmo2010_monitoringinteractactivity', ['monitoring_id', 'user_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'MonitoringInteractActivity', fields ['monitoring', 'user']
        db.delete_unique('exmo2010_monitoringinteractactivity', ['monitoring_id', 'user_id'])
