...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        orm.InviteOrgs.objects.filter(inv_status='ALL').update(dst_orgs_noreg=True, dst_orgs_inact=True, dst_orgs_activ=True)
        orm.InviteOrgs.objects.filter(inv_status__in=['NTS', 'SNT', 'RD']).update(dst_orgs_noreg=True)
        orm.InviteOrgs.objects.filter(inv_status='RGS').update(dst_orgs_inact=True)
        orm.InviteOrgs.objects.filter(inv_status='ACT').update(dst_orgs_activ=True)
