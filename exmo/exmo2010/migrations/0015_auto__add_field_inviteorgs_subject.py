...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'InviteOrgs.subject'
        db.add_column('exmo2010_inviteorgs', 'subject',
                      self.gf('django.db.models.fields.TextField')(),
                      keep_default=False)
        if not db.dry_run:
            orm.InviteOrgs.objects.filter(subject='').update(subject=u'Приглашение к взаимодействию в системе Инфометр')


    def backwards(self, orm):
        # Deleting field 'InviteOrgs.subject'
        db.delete_column('exmo2010_inviteorgs', 'subject')
